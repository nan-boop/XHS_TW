...
| 用户意图 / 文案类型 | 默认引擎 | 说明 |
|---|---|---|
| `seeding` 种草分享、好物分享 | `zhongcao` | 单篇口语种草，标题和标签按种草规格 |
| `hybrid` 种草品宣综合 | `zhongcao` + 用户方案 | 种草结构，卖点只取已核实方案 |
| `official` 官方品宣、`launch` 新品发布、`faq`、`campaign` | 本 skill 品宣规格 | 按 [copy-types.md](copy-types.md) 写，不套种草人设 |
| `review`、`tutorial`、`listicle` | `zhongcao` 结构 + 类型规格 | 短段落，但不编测评体验 |
| 系统做品牌内容、账号定位、内容矩阵、内容日历或长期增长 | `xiaohongshu-skills` | 先做策略与内容系统，再落到单篇文案 |
| 同时需要品牌策略和一篇可发布笔记 | 先 `xiaohongshu-skills`，后按文案类型成稿 | 策略不冒充产品事实 |
| 用户明确点名某个引擎 | 按用户点名 | 点名优先，但仍须遵守类型规格和安全门 |
...
输入至少记录：文案类型、产品/主题、目标人群、使用场景、已确认参数与卖点、限制条件、禁用词、品牌口吻和是否需要配图/发布。

输出至少包含：文案类型、标题（必要时多版本）、正文、话题标签、配图或封面建议、事实来源与待确认项。涉及产品的内容必须先经过产品简报确认，再按选定类型成稿。
